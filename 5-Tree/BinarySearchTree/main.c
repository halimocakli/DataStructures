...
void LevelOrderTraversal(struct node* _root, int level, int*);
...
int main()
{
	...
	*/
	...
	{
		sum = 0;
		LevelOrderTraversal(mainRoot, i, &sum);
		printf("%d", sum);
	}
...
void LevelOrderTraversal(BTREE* _root, int level, int*sum)
{
	if (_root != NULL)
	{
		if (level == 1)
		...
		{
			LevelOrderTraversal(_root->leftNode, level - 1, sum);
			LevelOrderTraversal(_root->rightNode, level - 1, sum);
		}
	}
}
